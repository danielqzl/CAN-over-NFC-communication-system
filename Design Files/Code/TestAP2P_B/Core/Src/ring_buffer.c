...
{
	uint8_t packet_head_next;
	uint8_t frame_head_next;
	uint32_t index = 0;
	uint8_t i;
	static uint8_t stdMSG[11] = {0x08, 0x01, 0x03, 0x54, 0x45, 0x53, 0x54, 0x5F, 0x4D, 0x53, 0x47};

	if(frame_cnt_en)
	{
		index = r->packet_head*256;
		for(i=0;i<n;i++)
		{
			memcpy(r->buffer+index+11*i, stdMSG, 11);
		}
		//r->frame_cnt[r->packet_head] = n;
		r->cnt++;
		if(++r->packet_head == RING_MAX_PACKET)
		{
			r->packet_head = 0;
		}
		r->cnt+=n;
	...
	for(i=0; i<n; i++)
	{
		index = r->packet_head*256 + r->frame_head*11;
		frame_head_next = r->frame_head + 1;
		if(frame_head_next == PACKET_MAX_FRAME)
		{
			frame_head_next = 0;
			if(++packet_head_next == RING_MAX_PACKET)
			{
				platformLog("Ring Overflow \r\n");
				return;
			}
		}
		memcpy(r->buffer+index, stdMSG, 11);
		r->frame_head = frame_head_next;
		r->packet_head = packet_head_next;
		r->cnt++;
	}
